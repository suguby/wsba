...
class AnswerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.answer_1 = Answer.objects.\
            create(question=self.question, position=1, text='test1',
                   is_right=False, has_comment=True)
        self.answer_2 = Answer.objects.\
            create(question=self.question, position=2, text='test2',
                   is_right=False, has_comment=False)
        self.answer_3 = Answer.objects.\
            create(question=self.question, position=3, text='test3',
                   is_right=False, has_comment=False)

    def test_previous_answer(self):
        question = Question.objects.create(text='test question2',
                                           answers_type='single')
        answer = Answer.objects.\
            create(question=question, position=4, text='test1',
                   is_right=False, has_comment=False)
    ...
    def test_answer_next(self):
        question = Question.objects.create(text='test question2',
                                           answers_type='single')
        answer = Answer.objects.\
            create(question=question, position=4,
                   text='test1', is_right=False, has_comment=False)
