...
class SlideEditViewTests(BaseTests):
    ...
    def test_template(self):
        ...
        self.assertContains(self.response, 'Редактирование слайда', status_code=200)
        self.assertContains(self.response, self. presentation.name, status_code=200)
        self.assertContains(self.response, self.slide.description, status_code=200)
    ...
    def test_post_edit_object(self):
        self.client.post(self.url, {'description': 'test edit',
                                    'presentation': self.presentation.id})
        self.assertEqual(CoreSlide.objects.filter(description='test edit').count(), 1)
...
class CoreSlideAddViewTests(BaseTests):
    ...
    def test_template(self):
        ...
        self.assertIn('form', self.response.context)
        self.assertInHTML(needle=hidden_input, haystack=self.response.rendered_content)
        self.assertEqual(self.response.context['presentation'].pk, self.presentation.id)
        self.assertContains(self.response, 'Создать', status_code=200)
        self.assertContains(self.response, 'Добавление слайда', status_code=200)
        self.assertContains(self.response, self.presentation.name, status_code=200)
        self.assertIn('back_button', self.response.context)
        self.assertNotIn('add_button', self.response.context)
        self.assertNotIn('edit_button', self.response.context)
    ...
    def test_post_edit_object(self):
        self.client.post(self.url, {'description': '12345',
                                    'presentation': self.presentation.id})
        self.assertEqual(CoreSlide.objects.filter(description='12345').count(), 1)
