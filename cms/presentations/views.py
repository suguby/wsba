...
class PresentationListView(LoginRequiredMixin,
                           ListView, BasePresentationsView):
    ...
    def get_queryset(self):
        ...
        return Presentation.objects.filter(organisation=organisation).\
            order_by('position')
...
class PresentationDetailView(LoginRequiredMixin,
                             DetailView, BasePresentationsView):
    ...
    def get_context_data(self, **kwargs):
        context = \
            super(PresentationDetailView, self).get_context_data(**kwargs)
        context['slide_list'] = \
            CoreSlide.objects.filter(presentation=self.kwargs['presentation'])
        return context


class PresentationCreateView(LoginRequiredMixin,
                             CreateView, BasePresentationsView):
    ...
    def get_initial(self):
        ...
        initial_new['organisation'] = Organisation.objects.\
            get(slug=self.kwargs['organisation'])
        return initial_new

    def get_success_url(self):
        return reverse('cms:presentations_list',
                       kwargs={'organisation': self.kwargs['organisation']})


class PresentationUpdateView(LoginRequiredMixin,
                             UpdateView, BasePresentationsView):
...
class PresentationDeleteView(LoginRequiredMixin,
                             DeleteView, BasePresentationsView):
