...
class PresentationListViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.presentation_2 = Presentation.objects.\
            create(name='p2', organisation=self.organisation)
        self.url = reverse('cms:presentations_list',
    ...
    def test_test_template(self):
        ...
        success_url = reverse('cms:presentations_add',
                              kwargs={'organisation': self.organisation.slug})
        self.assertTemplateUsed(self.response, 'cms/presentations/list.html')
        self.assertEqual(len(presentations),
                         len(self.response.context['object_list']))
...
class PresentationDetailViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.presentation = Presentation.objects.\
            create(name='p1', organisation=self.organisation)
        self.url = reverse('cms:presentations_detail',
                           kwargs={'organisation': self.organisation.slug,
    ...
    def test_not_get_object(self):
        url_404 = reverse('cms:presentations_detail',
                          kwargs={'organisation': self.organisation.slug,
                                  'presentation': 100})
    ...
    def test_template(self):
        url_back = reverse('cms:presentations_list',
                           kwargs={'organisation': self.organisation.slug})
        url_add = reverse('cms:slides_add',
                          kwargs={'organisation': self.organisation.slug,
                                  'presentation': self.presentation.id})
        url_edit = reverse('cms:presentations_edit',
                           kwargs={'organisation': self.organisation.slug,
                                   'presentation': self.presentation.id})
        url_delete = reverse('cms:presentations_delete',
                             kwargs={'organisation': self.organisation.slug,
                                     'presentation': self.presentation.id})
...
class PresentationCreateViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.presentation = Presentation.objects.\
            create(name='p1', organisation=self.organisation)
        self.url = reverse('cms:presentations_add',
                           kwargs={'organisation': self.organisation.slug})
        self.response = self.client.get(self.url)
    ...
    def test_template(self):
        ...
        self.assertContains(self.response, 'Добавление презентации',
                            status_code=200)
        self.assertIn('back_button', self.response.context)
    ...
    def test_post_status_valid(self):
        url = reverse('cms:presentations_add',
                      kwargs={'organisation': self.organisation.slug})
        response = self.client.\
            post(url, {'name': 'new', 'organisation': self.organisation.id})
    ...
    def test_post_add_object(self):
        ...
        url = reverse('cms:presentations_add',
                      kwargs={'organisation': self.organisation.slug})
        self.client.post(url, {'name': 'new',
                               'organisation': self.presentation.id})
...
class PresentationEditViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.presentation = Presentation.objects.\
            create(name='test', organisation=self.organisation)
        self.url = reverse('cms:presentations_edit',
                           kwargs={'organisation': self.organisation.slug,
                                   'presentation': self.presentation.id})
        self.response = self.client.get(self.url)
    ...
    def test_template(self):
        self.assertEqual(self.presentation.id,
                         self.response.context['object'].id)
        self.assertTemplateUsed(self.response, 'cms/presentations/edit.html')
        self.assertContains(self.response, 'Обновить', status_code=200)
        self.assertContains(self.response, 'Редактирование презентации',
                            status_code=200)
        self.assertContains(self.response, self.presentation.name,
                            status_code=200)
        self.assertContains(self.response, self.presentation.position,
                            status_code=200)
        self.assertIn('back_button', self.response.context)
        self.assertNotIn('add_button', self.response.context)
        self.assertNotIn('edit_button', self.response.context)
    ...
    def test_post_status_valid(self):
        Presentation.objects.create(name='test123',
                                    organisation=self.organisation)
        url = reverse('cms:presentations_edit',
                      kwargs={'organisation': self.organisation.slug,
                              'presentation': self.presentation.id})
        response = \
            self.client.post(url, {'name': 'edit test123',
                                   'organisation': self.organisation.id})
        self.assertEquals(response.status_code, 302)

    def test_post_status_invalid(self):
        presentation = Presentation.objects.\
            create(name='test update', organisation=self.organisation)
        url = reverse('cms:presentations_edit',
                      kwargs={'organisation': self.organisation.slug,
    ...
    def test_post_edit_object(self):
        presentation = Presentation.objects.\
            create(name='for edit', organisation=self.organisation)
        url = reverse('cms:presentations_edit',
                      kwargs={'organisation': self.organisation.slug,
                              'presentation': presentation.id})
        self.client.\
            post(url, {'name': 'edited', 'organisation': self.organisation.id})
        self.assertEqual(Presentation.objects.
                         filter(name='for edit').count(), 0)
...
class PresentationDeleteViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.presentation = Presentation.objects.\
            create(name='test delete p', organisation=self.organisation)
        self.presentation_2 = Presentation.objects.\
            create(name='test delete p2', organisation=self.organisation)
        self.presentation_3 = Presentation.objects.\
            create(name='test delete p3', organisation=self.organisation)
        self.presentation_3 = Presentation.objects.\
            create(name='test delete p4', organisation=self.organisation)
        self.url = reverse('cms:presentations_delete',
                           kwargs={'organisation': self.organisation.slug,
    ...
    def test_post_status_invalid(self):
        url = reverse('cms:presentations_delete',
                      kwargs={'organisation': self.organisation.slug,
                              'presentation': 10})
    ...
    def test_delete_object(self):
        self.assertEqual(Presentation.objects.count(), 3)
        self.assertEqual(Presentation.objects.
                         filter(name='test delete p4').count(), 0)
