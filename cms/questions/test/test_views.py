...
class QuestionListViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.question = Question.objects.create(text='test3',
                                                answers_type='single')
        self.question_2 = Question.objects.create(text='test',
                                                  answers_type='single')
        self.question_3 = Question.objects.create(text='test2',
                                                  answers_type='single')
        self.url = reverse('cms:questions_list',
    ...
    def test_test_template(self):
        ...
        success_url = reverse('cms:questions_add',
                              kwargs={'organisation': self.organisation.slug})
...
class QuestionDetailViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.url = reverse('cms:questions_detail',
                           kwargs={'organisation': self.organisation.slug,
    ...
    def test_not_get_object(self):
        url_404 = reverse('cms:questions_detail',
                          kwargs={'organisation': self.organisation.slug,
                                  'question': 100})
    ...
    def test_template(self):
        url_back = reverse('cms:questions_list',
                           kwargs={'organisation': self.organisation.slug})
        url_add = reverse('cms:answers_add',
                          kwargs={'organisation': self.organisation.slug,
                                  'question': self.question.id})
        url_edit = reverse('cms:questions_edit',
                           kwargs={'organisation': self.organisation.slug,
                                   'question': self.question.id})
        url_delete = reverse('cms:questions_delete',
                             kwargs={'organisation': self.organisation.slug,
                                     'question': self.question.id})
...
class QuestionCreateViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.question = Question.objects.\
            create(text='test update', answers_type='single')
        self.url = reverse('cms:questions_add',
                           kwargs={'organisation': self.organisation.slug})
        self.response = self.client.get(self.url)
    ...
    def test_template(self):
        ...
        self.assertContains(self.response, 'Добавление вопроса',
                            status_code=200)
        self.assertIn('back_button', self.response.context)
    ...
    def test_post_add_object(self):
        ...
        self.client.post(url,
                         {'text': 'new_question', 'answers_type': 'multi'})
        self.assertEqual(Question.objects.count(), 3)
        self.assertEqual(Question.objects.
                         filter(text='new_question').count(), 1)
...
class QuestionEditViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.question = Question.objects.\
            create(text='test update', answers_type='single')
        self.url = reverse('cms:questions_edit',
                           kwargs={'organisation': self.organisation.slug,
                                   'question': self.question.id})
        self.response = self.client.get(self.url)
    ...
    def test_template(self):
        ...
        self.assertContains(self.response, 'Редактирование вопроса',
                            status_code=200)
        self.assertContains(self.response, self.question.text, status_code=200)
        self.assertContains(self.response, self.question.answers_type,
                            status_code=200)
        self.assertIn('back_button', self.response.context)
        self.assertNotIn('add_button', self.response.context)
        self.assertNotIn('edit_button', self.response.context)
    ...
    def test_post_status_valid(self):
        question = Question.objects.create(text='test update',
                                           answers_type='single')
        url = reverse('cms:questions_edit',
                      kwargs={'organisation': self.organisation.slug,
                              'question': question.id})
        response = self.client.post(url, {'text': 'edit question',
                                          'answers_type': 'multi'})
        self.assertEquals(response.status_code, 302)
    ...
    def test_post_edit_object(self):
        question = Question.objects.\
            create(text='testing', answers_type='single')
        url = reverse('cms:questions_edit',
                      kwargs={'organisation': self.organisation.slug,
                              'question': question.id})
        self.client.post(url, {'text': 'edit question',
                               'answers_type': 'multi'})
...
class QuestionDeleteViewTests(BaseTests):
    ...
    def setUp(self):
        ...
        self.question = Question.objects.\
            create(text='test delete', answers_type='single')
        self.question_2 = Question.objects.\
            create(text='test delete 2', answers_type='single')
        self.question_3 = Question.objects.\
            create(text='test delete 3', answers_type='single')
        self.url = reverse('cms:questions_delete',
                           kwargs={'organisation': self.organisation.slug,
                                   'question': self.question.id})
    ...
    def test_post_status_invalid(self):
        url = reverse('cms:questions_delete',
                      kwargs={'organisation': self.organisation.slug,
                              'question': 10})
    ...
    def test_delete_object(self):
        self.assertEqual(Question.objects.count(), 2)
        self.assertEqual(Question.objects.
                         filter(text='test delete').count(), 0)
